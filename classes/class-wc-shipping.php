...
class WC_Shipping {
	...
	/** @var array Stores the info of shipping*/
	var $shipping_info 			= array();

	/** @var float Stores the cost of shipping (As it see Customer)*/
	var $shipping_total 			= 0;

	/** @var float Stores the Real cost of shipping you spended for*/
	...
	function calculate_shipping( $packages = array() ) {
		...
		$this->shipping_total_real= 0;
		$this->shipping_info	= array();
		$this->shipping_taxes 	= array();
		$this->shipping_label 	= null;
		$this->packages		    = array();
		$_cheapest_cost = $_cheapest_method = $chosen_method = $chosen_method_variant = $chosen_method_variant_label = $chosen_method_variant_cost = $chosen_method_variant_description = '';
		...
		if ( sizeof( $_available_methods ) > 0 ) {
			...
			if ( $chosen_method ) {
				...
				$this->shipping_total_real = $_available_methods[ $chosen_method ]->cost_real;
				$this->shipping_taxes 	= $_available_methods[ $chosen_method ]->taxes;
				$this->shipping_info	= $_available_methods[ $chosen_method ]->info;
