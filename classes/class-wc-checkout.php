...
class WC_Checkout {
	...
	public function create_order() {
		...
		//Lets methods do somethig for checkout
		//do_action( 'woocommerce_checkout_order_placed_' . $this->shipping_method->id, $order_id, $this->shipping_method );
		...
		if ( $this->shipping_method ) {
			...
			update_post_meta( $order_id, '_order_shipping_real', 	woocommerce_format_total($this->shipping_method->cost_real) );
			update_post_meta( $order_id, '_order_shipping_info',    $this->shipping_method->info );
			update_post_meta( $order_id, '_order_shipping_tracking', $this->shipping_method->info['tracking_number'] );
			update_post_meta( $order_id, '_order_shipping_tracking_label', $this->shipping_method->info['tracking_number_label'] );
			update_post_meta( $order_id, '_order_shipping_tracking_status', $this->shipping_method->info['tracking_number_status'] );
			update_post_meta( $order_id, '_order_shipping_customer_instruction', $this->shipping_method->info['instruction'] );
		}
