...
class WC_Cart {
	...
	/*-----------------------------------------------------------------------------------*/
		...
		public function get_cart_from_session() {
			...
			// Load the cart
			//unset($woocommerce->session->cart);
			...
			$this->shipping_total_real	= isset( $woocommerce->session->shipping_total_real ) ? $woocommerce->session->shipping_total_real : 0;
			$this->shipping_info 		= isset( $woocommerce->session->shipping_info ) ? $woocommerce->session->shipping_info : 0;
			$this->discount_totals 		= isset( $woocommerce->session->discount_totals ) ? $woocommerce->session->discount_totals : 0;
			$this->cached			= array();
		...
		public function set_session() {
			...
			$woocommerce->session->shipping_total_real  = $this->shipping_total_real;
			$woocommerce->session->shipping_info        = $this->shipping_info;
		...
		public function get_item_data( $cart_item, $flat = false ) {
			...
			if ( ! empty( $cart_item['data']->variation_id ) ) {
				...
				foreach ( $attributes as $attribute ) {
					...
					if ( $flat )
						$variation_list[] = $woocommerce->attribute_label( $attribute['name'] ) . ': ' . $value;
					else
						$variation_list[] = '<dt>' . $woocommerce->attribute_label( $attribute['name'] ) . ':</dt><dd>' . $value . '</dd>'; 
	...
	/*-----------------------------------------------------------------------------------*/
		...
		public function calculate_shipping() {
			...
			$this->shipping_total_real	= $woocommerce->shipping->shipping_total_real;	// Shipping Real Total
			$this->shipping_info 		= $woocommerce->shipping->shipping_info;	// Shipping Info
