							...
							echo '
								<tr class="' . esc_attr( apply_filters('woocommerce_checkout_table_item_class', 'checkout_table_item', $values, $cart_item_key ) ) . '">
									...
									if( isset($values['line_total_discounted']) && ( $_product->get_price() * $values['quantity'] ) != $values['line_total_discounted'] * $values['quantity'] ) {
										echo '<del>' .
										     apply_filters( 'woocommerce_checkout_item_subtotal', $woocommerce->cart->get_product_subtotal( $_product, $values['quantity'] ), $values, $cart_item_key ) .
										     '</del>' .
										     '<ins>' .
										     woocommerce_price( $values['line_total_discounted'] * $values['quantity'] ) .
										     '</ins>';
									} else {
//										echo $woocommerce->cart->get_product_subtotal( $_product, $values['quantity'] );
										echo apply_filters( 'woocommerce_checkout_item_subtotal', $woocommerce->cart->get_product_subtotal( $_product, $values['quantity'] ), $values, $cart_item_key );
									}
									echo '</td>
								</tr>';
						endif;
