...
if ( ! function_exists( 'woocommerce_page_title' ) ) {
	...
	function woocommerce_page_title() {
		...
		if ( is_search() ) {
			$page_title = '<span class="search result header">' . sprintf( __( 'Search Results:', 'woocommerce' ) . ' &ldquo;%s&rdquo;', get_search_query() ) . '</span>';

			if ( get_query_var( 'paged' ) )
				$page_title .= sprintf( '&nbsp;&ndash; ' . __( 'Page', 'woocommerce' ) . ' %s', get_query_var( 'paged' ) );
...
if ( ! function_exists( 'woocommerce_default_product_tabs' ) ) {
	...
	function woocommerce_default_product_tabs( $tabs = array() ) {
		...
		if ( comments_open() )
			...
			$tabs['reviews'] = array(
				'title'    => sprintf( _n('%d Review', '%d Reviews', $review_count, 'woocommerce' ), $review_count ),
				'priority' => 30,
...
if ( ! function_exists( 'woocommerce_template_single_shipping' ) ) {
    function woocommerce_template_single_shipping() {
