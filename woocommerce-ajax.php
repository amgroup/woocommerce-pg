	...
	if ( isset( $data['attribute_names'] ) ) {
		...
		if ( isset( $data['attribute_variation'] ) )
			$attribute_variation = $data['attribute_variation'];
                ...
                if ( isset( $data['attribute_changeable'] ) )
			$attribute_changeable = $data['attribute_changeable'];
		...
		for ( $i=0; $i < $attribute_names_count; $i++ ) {
			...
			$is_variation 	= !empty( $attribute_variation[ $i ] ) ? 1 : 0;
                        $is_changeable 	= !empty( $attribute_changeable[ $i ] ) ? 1 : 0;
...
/*
 *
 * AJAX dynamic update product variations values (jquery)
 */

if( !function_exists( 'get_data_product_variations' ) ) {
    function get_data_product_variations() {

        global $woocommerce, $product, $post;

        if ( ! empty($_POST['product_id']) && preg_match('/^([\d]+)$/', $_POST['product_id']) ) {

            $post = get_post( $_POST['product_id'] );
            if ( $post && $post->post_type === 'product' ) {
                header( 'Content-Type: text/plain; charset=UTF-8' );

                $product  = new WC_Product_Variable( $post );
    ...
    add_action( 'wp_ajax_nopriv_get_data_product_variations', 'get_data_product_variations' );
    add_action( 'wp_ajax_get_data_product_variations', 'get_data_product_variations' );
}
...
if( !function_exists( 'get_woocommerce_nonce_params' ) ) {
    ...
    add_action( 'wp_ajax_get_woocommerce_nonce_params', 'get_woocommerce_nonce_params' );
}
...
if( !function_exists( 'checkout_shipping_form' ) ) {
	...
	add_action( 'wp_ajax_nopriv_checkout_shipping_form', 'checkout_shipping_form' );
	add_action( 'wp_ajax_checkout_shipping_form', 'checkout_shipping_form' );
}
