...
function get_woocommerce_currencies() {
	return array_unique(
		apply_filters( 'woocommerce_currencies',
			array(
				'RUB' => __( 'Russian Rubles (RUB, new currency code 643, since 2001)', 'woocommerce' ),
				'RUR' => __( 'Russian Rubles (RUR, old currency code 810, before 2001)', 'woocommerce' ),
...
function woocommerce_init_roles() {
	...
	if ( is_object( $wp_roles ) ) {
		...
		// Customer role
		add_role( 'shop_customer', __( 'Shop Сustomer', 'woocommerce' ), array(
		    'read' 					=> true,
		    'edit_posts' 				=> false,
		    'delete_posts' 				=> false
		) );
		...
		foreach( $capabilities as $cap_group ) {
			foreach( $cap_group as $cap ) {
				$wp_roles->add_cap( 'shop_administrator', $cap );
...
function woocommerce_remove_roles() {
	...
	if ( is_object( $wp_roles ) ) {
		...
		foreach( $capabilities as $cap_group ) {
			foreach( $cap_group as $cap ) {
				$wp_roles->remove_cap( 'shop_administrator', $cap );
				$wp_roles->remove_cap( 'shop_manager', $cap );
				$wp_roles->remove_cap( 'administrator', $cap );
			}
		}

		remove_role( 'shop_customer' );
		remove_role( 'shop_manager' );
		remove_role( 'shop_administrator' );
	}
}
...
if( !function_exists( 'mb_initcap' ) ) {
    function mb_initcap ($mb_string, $charset='utf-8') {
        return mb_substr(mb_strtoupper($mb_string, $charset), 0, 1, $charset) . mb_substr( $mb_string, 1, strlen($mb_string), $charset);
    }
}

if( !function_exists( 'time_in_interval' ) ) {
    function time_in_interval($from, $to, $now=null) {
        $in_interval = 0;
        
        if( !isset($now) ) {
            $_now = getdate();
            //LOCAL (MOSCOW) UTC +4 OFFSET
            $_now['hours'] += 4;
            $now  = implode( ":", array( $_now['hours'], $_now['minutes'], $_now['seconds']));
        }
        $now = time_for_cmp( $now );

        if( isset($from) && isset($to) ) {
            $from = time_for_cmp( $from );
            $to   = time_for_cmp( $to );
...
if( !function_exists( 'time_for_cmp' ) ) {
    function time_for_cmp( $time ) {
        $_time = split(':', $time);
...
if( !function_exists( 'product_page_title' ) ) {
	function product_page_title() {
		...
		foreach( get_the_terms( $post->ID, 'product_cat' ) as $_cat ) {
			if( preg_match( '/\/' . $_cat->slug . '\//', $current_url, $matches) ) {
				$cat_title = esc_html( strip_tags( $_cat->description ? $_cat->description : $_cat->name ) );
				if( ! in_array( $cat_title, $cat_titles ) )
					$cat_titles[] = $cat_title;
			}
		}
		$cat_titles[] = esc_html( strip_tags( $post->post_title ) );
