		...
		<?php else : ?>
			...
			<p class="toolbar">
				...
				<select id="field_to_edit">
					...
					<option value="variable_purchase_price"><?php _e( 'Purchase Prices', 'woocommerce' ); ?></option>
					...
					?>
							<option value="attribute_<?php echo sanitize_title( $attribute['name'] );?>" data-values="<?php echo implode(', ', $attribute_values); ?>" rel="select"><?php _e( 'Change', 'woocommerce' ); ?> <?php echo esc_html( $woocommerce->attribute_label( $attribute['name'] ) ); ?></option>
					<?php
						}
					}
					?>
...
console.log(input_tag + '[name^="' + field_to_edit + '["]');
				var message = "<?php _e( 'Enter a value', 'woocommerce' );?>";
				if( jQuery('select#field_to_edit :selected').attr('data-values') )
					message += ". " + "<?php _e( 'One from: ', 'woocommerce' );?>" + jQuery('select#field_to_edit :selected').attr('data-values');
...
function process_product_meta_variable( $post_id ) {
	...
	if ( isset( $_POST['variable_sku'] ) ) {
		...
		for ( $i = 0; $i <= $max_loop; $i ++ ) {
			...
			// Price handling
			$purchase_price 	= woocommerce_clean( $variable_purchase_price[ $i ] );
