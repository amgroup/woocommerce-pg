...
function woocommerce_order_totals_meta_box( $post ) {
	...
	<div class="totals_group">
		...
		<ul class="totals">
			...
			<li class="wide">
				<?php
					$instruction = maybe_unserialize( $data['_order_shipping_customer_instruction'][0] );
					if( is_array($instruction) ) {
						$instruction = (string) implode("\n", $instruction );
					}
					esc_textarea($instruction);

					woocommerce_wp_textarea_input( array(
						'id'          => '_order_shipping_customer_instruction',
						'class'       => 'large disabled',
						'label'       => __( 'Instruction (for Customer):', 'woocommerce' ),
						'description' => __( 'Pickup Instruction', 'woocommerce' ),
						'value'       => $instruction,
						'name'        => '_order_shipping_customer_instruction',
						'rows'        => 5,
						'readonly'    => 'readonly',
					) );
				?>
			...
			<li class="left">
				<label><?php _e( 'Cost (for Customer):', 'woocommerce' ); ?></label>
			...
			<li class="right">
				<label><?php _e( 'Cost (Real):', 'woocommerce' ); ?></label>
				<input type="number" step="any" min="0" id="_order_shipping_real" name="_order_shipping_real" placeholder="0.00 <?php _e( '(ex. tax)', 'woocommerce' ); ?>" value="<?php
					if ( isset( $data['_order_shipping_real'][0] ) )
						echo esc_attr( $data['_order_shipping_real'][0] );
				?>" class="first" />
			</li>

			<li class="left">
				<label><?php _e( 'Tracking Number:', 'woocommerce' ); ?></label>
				<input type="text" id="_order_shipping_tracking" name="_order_shipping_tracking" placeholder="" pattern="([^а-яА-Я]+)" value="<?php
					if ( isset( $data['_order_shipping_tracking'][0] ) )
						echo esc_attr( $data['_order_shipping_tracking'][0] );
				?>" class="first" />
			</li>

			<li class="right">
				<label><?php _e( 'Label for Tracking Number:', 'woocommerce' ); ?></label>
				<input type="text" id="_order_shipping_tracking_label" name="_order_shipping_tracking_label" placeholder="" value="<?php
					if ( isset( $data['_order_shipping_tracking_label'][0] ) )
						echo esc_attr( $data['_order_shipping_tracking_label'][0] );
				?>" class="first" />
			</li>

			<li class="left">
				<label><?php _e( 'Shipping Status:', 'woocommerce' ); ?></label>
				<input type="text" id="_order_shipping_tracking_status" name="_order_shipping_tracking_status" placeholder="<?php _e( 'Created', 'woocommerce' ); ?>" value="<?php
					if ( isset( $data['_order_shipping_tracking_status'][0] ) )
						echo esc_attr( $data['_order_shipping_tracking_status'][0] );
...
function woocommerce_process_shop_order_meta( $post_id, $post ) {
	...
	update_post_meta( $post_id, '_shipping_email', woocommerce_clean( $_POST['_shipping_email'] ) );
	update_post_meta( $post_id, '_shipping_phone', woocommerce_clean( $_POST['_shipping_phone'] ) );
	update_post_meta( $post_id, '_order_shipping', woocommerce_clean( $_POST['_order_shipping'] ) );
	update_post_meta( $post_id, '_order_shipping_real', woocommerce_clean( $_POST['_order_shipping_real'] ) );
	update_post_meta( $post_id, '_order_shipping_tracking', woocommerce_clean( $_POST['_order_shipping_tracking'] ) );
	update_post_meta( $post_id, '_order_shipping_tracking_status', woocommerce_clean( $_POST['_order_shipping_tracking_status'] ) );
	update_post_meta( $post_id, '_order_shipping_tracking_label', woocommerce_clean( $_POST['_order_shipping_tracking_label'] ) );
