...
<div class="woocommerce_variation wc-metabox closed">
	<h3>
		...
		<strong>
			...
			<?php if ( isset( $_regular_price ) ) echo ' ('. woocommerce_price( $_regular_price ) . ')'; ?>
			&#032;&mdash;&#032;
		</strong>
		...
		<?php
			...
			foreach ( $parent_data['attributes'] as $attribute ) {
				...
				echo '<select name="attribute_' . sanitize_title( $attribute['name'] ) . '[' . $loop . ']"><option value="">' . esc_html( $woocommerce->attribute_label( $attribute['name'] ) ) . '&hellip;</option>';
	...
	<table cellpadding="0" cellspacing="0" class="woocommerce_variable_attributes wc-metabox-content">
			...
			<tr>
				...
				<td class="data" rowspan="2">
					<table cellspacing="0" cellpadding="0" class="data_table">
						...
						<tr>
							<td>
								<label><?php echo __( 'Purchase Price:', 'woocommerce' ) . ' ('.get_woocommerce_currency_symbol().')'; ?></label>
								<input type="number" size="5" name="variable_purchase_price[<?php echo $loop; ?>]" value="<?php if ( isset( $_purchase_price ) ) echo esc_attr( $_purchase_price ); ?>" step="any" min="0" placeholder="<?php _e( 'Variation purchase price', 'woocommerce' ); ?>" />
							</td>
							<td>
							</td>
						</tr>
		...
		<?php
			foreach ( $parent_data['attributes'] as $attribute ) {
				...
				// Name will be something like attribute_pa_color
				echo '<p>';
				echo '<label for="attribute_' . sanitize_title( $attribute['name'] ) . '[' . $loop . ']"><strong>'. esc_html( $woocommerce->attribute_label( $attribute['name'] ) ) . ': </strong></label><select name="attribute_' . sanitize_title( $attribute['name'] ) . '[' . $loop . ']"><option value="">' . __( 'Any', 'woocommerce' ) .'&hellip;</option>';
