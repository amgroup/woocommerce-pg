	...
	<div class="panel-wrap product_data">
		...
		<div id="general_product_data" class="panel woocommerce_options_panel"><?php
			...
			echo '<div class="options_group pricing show_if_simple show_if_variable show_if_external">';
				...
				// Price
				woocommerce_wp_text_input( array( 'id' => '_regular_price', 'class' => 'wc_input_price short', 'label' => __( 'Regular Price', 'woocommerce' ) . ' ('.get_woocommerce_currency_symbol().')', 'type' => 'text', 'custom_attributes' => array(
					'step' 	=> 'any',
					'min'	=> '0',
					'pattern'=>'([0-9]+)(\.[0-9]{1,2})?',
				) ) );
				...
				// Special Price
				woocommerce_wp_text_input( array( 'id' => '_sale_price', 'class' => 'wc_input_price short', 'label' => __( 'Sale Price', 'woocommerce' ) . ' ('.get_woocommerce_currency_symbol().')', 'description' => '<a href="#" class="sale_schedule">' . __( 'Schedule', 'woocommerce' ) . '</a>', 'type' => 'text', 'custom_attributes' => array(
					'step' 	=> 'any',
					'min'	=> '0',
					'pattern'=>'([0-9]+)(\.[0-9]{1,2})?',
				) ) );
				...
				// Special Price time range
				$sale_price_time_from 	= get_post_meta( $thepostid, '_sale_price_time_from', true );
				$sale_price_time_to 	= get_post_meta( $thepostid, '_sale_price_time_to', true );

				echo '	<p class="form-field sale_price_time_fields">
							<label for="_sale_price_time_from">' . __( 'Sale Price Time', 'woocommerce' ) . '</label>
							<input type="text" class="short" name="_sale_price_time_from" id="_sale_price_time_from" value="' . $sale_price_time_from . '" placeholder="' . _x( 'From&hellip;', 'placeholder', 'woocommerce' ) . ' HH:MM:SS" maxlength="8" pattern="([0-1]?[0-9]|2[0-4]):([0-5][0-9])(:[0-5][0-9])?" />
							<input type="text" class="short" name="_sale_price_time_to" id="_sale_price_time_to" value="' . $sale_price_time_to . '" placeholder="' . _x( 'To&hellip;', 'placeholder', 'woocommerce' ) . ' HH:MM:SS" maxlength="8" pattern="([0-1]?[0-9]|2[0-4]):([0-5][0-9])(:[0-5][0-9])?" />
						</p>';
		...
		<div id="inventory_product_data" class="panel woocommerce_options_panel">
			...
			$available_date = ( $date = get_post_meta( $thepostid, '_stock_status_date', true ) ) ? date_i18n( 'Y-m-d', $date ) : '';
			woocommerce_wp_text_input( array( 'id' => '_stock_status_date', 'label' => __( 'Available From', 'woocommerce' ), 'class' => 'short', 'description' => '', 'type' => 'date', 'placeholder' =>  _x( 'From&hellip;', 'placeholder', 'woocommerce' ), 'custom_attributes' => array(
		...
		<div id="shipping_product_data" class="panel woocommerce_options_panel">
			...
			echo '<div class="options_group">';
				...
				if( get_option('woocommerce_enable_weight', true) !== 'no' ) :
					woocommerce_wp_text_input( array( 'id' => '_weight', 'label' => __( 'Weight', 'woocommerce' ) . ' ('.get_option('woocommerce_weight_unit').')', 'placeholder' => '0.00', 'description' => __( 'Weight in decimal form', 'woocommerce' ), 'pattern'=>'([0-9]+)([,.][0-9]+)?', 'type' => 'numreic', 'custom_attributes' => array(
					'step' 	=> 'any',
					'min'	=> '0'
				) ) );
				else:
				    		...
				    		<div class="woocommerce_attribute wc-metabox closed taxonomy <?php echo $attribute_taxonomy_name; ?>" rel="<?php echo $position; ?>" <?php if ( ! $has_terms ) echo 'style="display:none"'; ?>>
								...
								<table cellpadding="0" cellspacing="0" class="woocommerce_attribute_data wc-metabox-content">
									<tbody>
										...
										<tr>
											<td>
												<div class="enable_variation show_if_variable enable_changeable" data-attribute="<?php echo $i; ?>">
												<label><input type="checkbox" class="checkbox" <?php
												...
												<label><input type="checkbox" class="checkbox" <?php
													if ( isset( $attribute['is_changeable'] ) )
														checked( $attribute['is_changeable'], 1 );
													else
														checked( apply_filters( 'default_attribute_changeable', false, $tax ), true );

												?> name="attribute_changeable[<?php echo $i; ?>]" value="1" /> <?php _e( 'Used for changeable', 'woocommerce' ); ?></label>
												</div>
...
function woocommerce_process_product_meta( $post_id, $post ) {
	...
	// Update post meta
	update_post_meta( $post_id, '_purchase_price', stripslashes( $_POST['_purchase_price'] ) );
	...
	if ( isset( $_POST['attribute_names'] ) ) {
		...
		if ( isset( $_POST['attribute_changeable'] ) )
			$attribute_changeable = $_POST['attribute_changeable'];
		...
		for ( $i=0; $i < $attribute_names_count; $i++ ) {
			...
			$is_changeable 	= isset( $attribute_changeable[ $i ] ) ? 1 : 0;
	...
	if ( in_array( $product_type, array( 'grouped' ) ) ) {
		...
		update_post_meta( $post_id, '_sale_price_time_from', '' );
		update_post_meta( $post_id, '_sale_price_time_to', '' );
		...
		$time_from = isset( $_POST['_sale_price_time_from'] ) ? $_POST['_sale_price_time_from'] : '';
		$time_to = isset( $_POST['_sale_price_time_to'] ) ? $_POST['_sale_price_time_to'] : '';
		...
		if ( $_POST['_sale_price'] != '' && $date_to == '' && $date_from == '' && $time_to == '' && $time_from == '')
		...
		if ( $_POST['_sale_price'] != '' && ( time_in_interval( $time_from, $time_to ) || ($date_from && strtotime( $date_from ) < strtotime( 'NOW', current_time( 'timestamp' ) ) ) ) )
			update_post_meta( $post_id, '_price', stripslashes($_POST['_sale_price']) );

		if ( ! time_in_interval( $time_from, $time_to ) && $date_to && strtotime( $date_to ) < strtotime( 'NOW', current_time( 'timestamp' ) ) ) {
			update_post_meta( $post_id, '_price', stripslashes($_POST['_regular_price']) );
			update_post_meta( $post_id, '_sale_price_dates_from', '');
			update_post_meta( $post_id, '_sale_price_dates_to', '');
		}
		...
		// Times
		update_post_meta( $post_id, '_sale_price_time_from', $time_from );
		update_post_meta( $post_id, '_sale_price_time_to', $time_to );
	}
	...
	if ( get_option('woocommerce_manage_stock') == 'yes' ) {
		...
		if ( $product_type == 'grouped' ) {
			...
			update_post_meta( $post_id, '_stock_status_date', stripslashes( $_POST['_stock_status_date'] ) );
	...
	process_product_synonyms_box_meta( $post_id );
